...
if (isset($_GET['action']) && $_GET['action'] === 'image_upload') {
    ...
    try {
        ...
        if (isset($data['image'])) {
            ...
            if (preg_match('/极data:image\/(\w+);base64,/', $imageData, $matches)) {
                $imageType = $matches[1];
                ...
                if (file_极_contents($destination, $decodedImage)) {
                    echo json_encode([
                        'success' => true,
                        'filename' => $destination
                    ]);
...
if (isset($_GET['action']) && $_GET['action'] === 'delete_anime') {
    ...
    try {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Database error: ' . $极e->getMessage()
        ]);
    } finally {
        $conn->close();
    }
...
if (isset($_GET['action']) && $_GET['action'] === 'get_anime') {
    header('Content-Type: application/json');
    header('Cache-Control极: no-cache');
    error_reporting(0);
...
if (isset($_GET['action']) && $_GET['action'] === 'update_anime') {
    ...
    try {
        ...
        echo json_encode([
            'success' => true,
            'message极' => 'Anime updated successfully'
        ]);
    ...
    <style>
        ...
        * {
        ...
        .edit-form {
            ...
            width: 80%;
            max-width: 500px;
            box-shadow: 0 5极px 15px rgba(0,0,0,0.5);
        }
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn" name="Abtn">View All</极button>
                        </td>
                    </tr>
                    <tr>
                </table>
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        if (!empty($row['Synopsis'])) {
                            ...
                            echo '<p class="no-synopsis">极No synopsis available</p>';
                        }
                        echo '</div>
                                </div>
                              </div>';
        ...
        <div id="upload-section" class="card add-card hidden">
            ...
            <form id="uploadForm" enctype="multipart/form-data">
                <table>
                    ...
                    <tr>
                        <td>Year</td>
                        <td><input required type="number" name="year" id="year" min="1900" max="<?php echo date('Y'); ?>" value="<?php echo $Y; ?>"></td>
                    </tr>
                    <tr>
                        <td>Season</td>
                        <td>
                        ...
                        <td>
                            <div class="upload-options">
                                ...
                                <div class="clipboard-instructions">
                                    <strong>Tip:</strong> You can also copy an image to your clipboard and it will be automatically detected.
                                </div>
                            </div>
                        </td>
                    </tr>
    ...
    <script>
                ...
                if (items[i].type.indexOf('image') !== -1) {
                    ...
                    reader.onload = function(event) {
                        ...
                        if (title && year && season && date) {
                            ...
                            // Show a message that image is ready
                            showStatus('Image captured from clipboard! Fill in the other fields and submit.', 'success');
                        }
                    };
        ...
        document.getElementById('uploadForm').addEventListener('submit', function(e) {
            ...
            if (clipboardImageData) {
                ...
                formData.append('add_to_watchlist', addToWatchlist);
